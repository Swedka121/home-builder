...
export function useEditor(
  ...
  useEffect(() => {
    ...
    if (canvasRef.current) {
      ...
      const skyboxMesh =
        skybox.getTheme(theme.theme || "light") || new THREE.Mesh();
      ...
      let timeoutSave = setInterval(() => {
        ...
        if (worldStore.save(worldId, editorStore.wallsData))
          toast("World autosave is success");
  ...
  useEffect(() => {
    ...
    threeState.current.renderer?.setClearColor(
      theme.theme == "dark" ? "#0a0a0a" : "#ffffff"
    );
    ...
    return () => {
      if (
        ...
        threeState.current.grid &&
        threeState.current.skyboxMesh
      ) {
  ...
  useEffect(() => {
    ...
    const getPosOnGrid = (event: MouseEvent, actionMode?: boolean) => {
      if (threeState.current.camera && threeState.current.gridPlane) {
        ...
        return points.filter(
          (value, index, self) =>
            index === self.findIndex((a) => a.x == value.x && a.z == value.z)
        );
      }
    };
    ...
    const mouseMoved = (event: MouseEvent) => {
      ...
      if (threeState.current.scene && points && points[0]) {
        ...
        const geometry = new THREE.BoxGeometry(0.2, 2.2, 0.2);
